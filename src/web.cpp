...
static void applyLowNoiseWifiProfile()
{
        WiFi.setSleep(true);
...
static void updateConfigFromForm(VarioConfig &cfg)
{
        ...
        {
                String pass = g_server.arg("sta_password");
                // Keep existing password when form submits an empty field.
                if (pass.length() > 0)
                        cfg.staPassword = pass;
        }
        if (g_server.hasArg("ap_password"))
        {
                String pass = g_server.arg("ap_password");
                if (pass.length() > 0)
                        cfg.apPassword = pass;
        }
...
<body>
                ...
                <section class="two">
                        <section class="panel">
                                ...
                                <form id="config-form" style="margin-top:12px;">
                                        <div class="fields">
                                                <div class="hint" style="grid-column:1/-1;">Modo FlySkyHy simplificado: ajuste apenas limiar de ascendente e limiar de descendente.</div>
                                                ...
                                                <label><span>Limiar de ascendente (m/s)</span><input name="cstart" type="number" step="0.01"><small class="help">Comeca o beep de ascendente quando o vario passa deste valor. Menor = apita mais cedo. Sugestao: 0.20.</small></label>
                                                ...
                                                <label><span>Tom base de ascendente (Hz)</span><input name="cbase" type="number" step="1"><small class="help">Frequencia inicial do beep de ascendente. Valor maior deixa o beep mais agudo.</small></label>
                                                <label><span>Ganho do tom de ascendente (Hz por m/s)</span><input name="cgain" type="number" step="1"><small class="help">Este e o parametro que faz o beep ficar mais agudo quanto maior o ascendente. Maior = muda mais rapido para agudo.</small></label>
                                                ...
                                                <label><span>Senha do AP (rede propria)</span><input name="ap_password" type="text" maxlength="32"><small class="help">Senha da rede VarioESP32 no modo AP. Minimo 8 caracteres.</small></label>
                                        </div>
                                        <div class="actions">
                                ...
                                <div style="margin-top:16px; padding-top:14px; border-top:1px solid var(--line);">
                                        <h3 style="margin:0 0 8px;">Simulador de vario (slider)</h3>
                                        <p class="muted" style="margin:0 0 10px;">Deslize para simular subida (+) ou descida (-). Solte para voltar ao sensor real.</p>
                                        <div style="display:flex; align-items:center; gap:12px;">
                                                <span style="font-weight:650; min-width:36px;">-7</span>
                                                <input type="range" id="vario-slider" min="-7" max="7" step="0.2" value="0" style="flex:1;">
                                                <span style="font-weight:650; min-width:36px;">+7</span>
                                                <strong id="slider-val" style="min-width:48px; text-align:center; font-size:1.1rem;">0.0</strong>
        ...
        <script>
                ...
                function setConfigForm(cfg) {
                        ...
                        form.ap_password.value = cfg.apPassword || '';
                }

                function hideConfig(name) {
                        const el = form[name];
                        if (!el) return;
                ...
                function simplifyFlySkyHyUi() {
                        const hiddenNames = ['dead', 'fast', 'adapt', 'autozero', 'cstop', 'sstop', 'sink_en', 'bpin', 'bdiv', 'bmin', 'bmax'];
                        hiddenNames.forEach(hideConfig);

                        const titles = document.querySelectorAll('.section-title');
                        titles.forEach((title) => {
                                if (title.textContent && title.textContent.trim() === 'Bateria') {
                ...
                // --- Slider simulador de vario ---
                const slider = document.getElementById('vario-slider');
                const sliderVal = document.getElementById('slider-val');
                let sliderTimer = null;

                slider.addEventListener('input', async () => {
                        const mps = parseFloat(slider.value);
                        sliderVal.textContent = mps.toFixed(1);
                        clearTimeout(sliderTimer);

                        // Envia o valor imediatamente ao deslizar
                        try {
                                await fetch('/api/test-vario?mps=' + mps, { method: 'POST' });
                        } catch (e) { /* ignora */ }
                });

                slider.addEventListener('change', async () => {
                        // Ao soltar, volta para 0 após 600ms (o backend já tem timeout)
                        sliderTimer = setTimeout(async () => {
                                slider.value = 0;
                                sliderVal.textContent = '0.0';
                                try {
                                        await fetch('/api/test-vario?mps=0', { method: 'POST' });
                                } catch (e) { /* ignora */ }
                        }, 800);
                });
...
static void handleTestSink()
{
        unsigned long durationMs = 6000;
        if (g_server.hasArg("ms"))
        {
                unsigned long requested = (unsigned long)g_server.arg("ms").toInt();
                if (requested >= 500 && requested <= 15000)
                        durationMs = requested;
        }

        soundTriggerSinkTest(durationMs);
        g_server.send(200, "application/json", String("{\"ok\":true,\"duration_ms\":") + String(durationMs) + "}");
}

static void handleTestClimb()
{
        ...
        if (g_server.hasArg("ms"))
        {
                unsigned long requestedMs = (unsigned long)g_server.arg("ms").toInt();
                if (requestedMs >= 500 && requestedMs <= 15000)
                        durationMs = requestedMs;
        }

        if (g_server.hasArg("mps"))
        {
                float requestedMps = g_server.arg("mps").toFloat();
                if (requestedMps >= 1.0f && requestedMps <= 7.0f)
                        climbMps = requestedMps;
        }

        soundTriggerClimbTest(climbMps, durationMs);
        String json = "{";
        json += "\"ok\":true";
        json += ",\"mps\":" + String(climbMps, 1);
        json += ",\"duration_ms\":" + String(durationMs);
        json += "}";
        g_server.send(200, "application/json", json);
}

static void handleTestVario()
{
        float mps = 0.0f;
        if (g_server.hasArg("mps"))
                mps = g_server.arg("mps").toFloat();

        soundSetSimulatedVario(mps);
        String json = "{\"ok\":true,\"mps\":" + String(mps, 1) + "}";
        g_server.send(200, "application/json", json);
}
...
static void startAp(const VarioConfig &cfg)
{
        WiFi.mode(WIFI_AP);
        applyLowNoiseWifiProfile();
        const char *pass = cfg.apPassword.c_str();
...
void webBegin()
{
        ...
        g_server.on("/api/test-sink", HTTP_POST, handleTestSink);
        g_server.on("/api/test-climb", HTTP_POST, handleTestClimb);
        g_server.on("/api/test-vario", HTTP_POST, handleTestVario);
        g_server.begin();
